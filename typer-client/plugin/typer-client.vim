function! Typer()
	...
	command! -nargs=0 Build execute "!typer ".expand("%")
	command! -nargs=0 Load call SendToPane("import final")
	"command! -nargs=0 Run execute "Build" | execute "Console" | execute "Load"
	command! -nargs=0 Run call RunFunction()
	command! -nargs=0 Init execute "Build" | execute "Console" | execute "AddRules" | execute "Load"

	xnoremap <buffer> <silent> <F5> y:call PasteToPane()<CR>

	command! Types call SendToPane("check A is type")
	command! Structs call SendToPane("check A is struct")
	command! Enums call SendToPane("check A is enum")
	command! -nargs=1 Info call SendToPane("check ".<q-args>." A B")
	command! AddRules call AddRulesFunction()
